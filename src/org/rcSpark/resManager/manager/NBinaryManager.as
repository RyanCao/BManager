...
public final class NBinaryManager{
    //-----------------------------------------
    //Var
    //-----------------------------------------
    private static var __instance : NBinaryManager;
     ...
     * see BinaryLoadQueue
     * */
    private var _loadQueueList:Vector.<BinaryLoadQueue>;
    /**
     * 等待加载库
     * see WaitToWake
     * */
    ...
    /**
     * 加载类型不能识别的加载类型都算在Other里面
     */
    private var loadTypes:Array = [
        [LoadType.LIB,3],
        [LoadType.CORE_FILES,3],
        [LoadType.AWAY3D_FILES,3],
        [LoadType.ICON,3],
        [LoadType.MAP_FILES,3],
        [LoadType.MOVIE,3],
        [LoadType.EXTERNAL_SWF_EFFECT,3],
        [LoadType.OTHER,3]
    ];
    public function NBinaryManager() {
        if(__instance){
            throw new Error("NBinaryManager is single!")
            return ;
        }
    ...
    public static function instance() : NBinaryManager {
        if (!__instance)
            __instance = new NBinaryManager();
        return __instance;
    }
    ...
    private function initLoadQueue():void{
        _loadQueueList = new Vector.<BinaryLoadQueue>();
        for(var i:int = 0,len:uint = loadTypes.length;i<len;i++){
            _loadQueueList.push(new BinaryLoadQueue(loadTypes[i][0],loadTypes[i][1]));
        }
    }
    /**
     * 使用单线程模式
     */
    rcant function useSingleThread():void{
        for(var i:int = 0,len:uint = _loadQueueList.length;i<len;i++){
            var lq:BinaryLoadQueue = _loadQueueList[i];
            lq.maxThread = 1 ;
        }
    }
    /**
     * 使用多线程模式
     */
    rcant function useMultiThread():void{
        for(var i:int = 0,len:uint = _loadQueueList.length;i<len;i++){
            var lq:BinaryLoadQueue = _loadQueueList[i];
            lq.maxThread = getLoadThread(lq.type) ;
        }
    }
     ...
     * @return
     */
    private function getLoadThread(type:uint):uint{
        var thread:int = 0 ;
        for(var i:int = 0,len:uint = loadTypes.length;i<len;i++){
            if(type ==loadTypes[i][0]){
                thread = loadTypes[i][1];
                break;
            }
        }
        return thread ;
     ...
     * @param loadType 加载类型 非Other的拥有各自的线程
     * @param loadLevel 加载优先级 当前加载类型下的优先级
    ...
    private function toLoad(url : Object,loadType:int,loadLevel:uint,onCompleteHandler : Function, onProgressHandler : Function, onErrorHandler : Function,isSave:Boolean) : void
    {
        var urlReq : URLRequest = urlHandle(url);
        if(!urlReq)
            return ;
    ...
    protected function startLoad() : void {
        ...
        while (i < _loadQueueList.length) {
            ...
            if(loadQueue.type!=LoadType.OTHER){
                ...
                allNoOtherThread += loadQueue.nowThread;
            }
            i++;
        }
        if(allNoOtherThread == 0){
            //非其他文件加载完成
    ...
    protected function onCompleteHandler(evt : BinaryEvent) : void {
        ...
        var loadQueue:BinaryLoadQueue = getLoadQueue(streamInfo.loadType);
        loadQueue.nowThread -- ;
    ...
    private function addUrlToWaitList(vo:BinaryInfo):void
    {
        ...
        if(!compair){
            ...
            while (i < _loadQueueList.length) {
                ...
                if(loadQueue.type!=LoadType.OTHER){
                    //再从高优先级的列表中查找
                    compair = loadQueue.getBinaryInfoByUrl(vo.url);
                    if(compair)
                        break;
                }
                i++;
        ...
        if(compair){
            ...
            //添加新url到等待队列中
            loadQueue = getLoadQueue(vo.loadType);
            loadQueue.addItem(vo);
        }
    }
     ...
     * @return
     */
    private function getLoadQueue(loadType:int):BinaryLoadQueue {
        var lq:BinaryLoadQueue ;
        for(var i:int = 0,len:uint = _loadQueueList.length;i<len;i++){
            if(_loadQueueList[i].type == loadType){
                lq = _loadQueueList[i];
                break;
            }
        }
        if(!lq){
            lq = getLoadQueue(LoadType.OTHER);
        }
        return lq ;
    }
    ...
    private function removeUrlFromWaitList(vo:BinaryInfo):void
    {
        ...
        var i:int = 0;
        while (i < _loadQueueList.length) {
            var loadQueue:BinaryLoadQueue = _loadQueueList[i];
            loadQueue.removeItem(vo);
            i++;
        }
    }
