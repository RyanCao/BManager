...
public class NewFileVersion implements IVersion
{
	 ...
	 * 版本串格式
	 */	
	public static var versionRegExp:RegExp = /[a-f0-9]{32}/;
	//-----------------------------------------------------------------------------
	// Constructor
	//-----------------------------------------------------------------------------
	...
	{
		var checkString:String = version.replace(versionRegExp,VersionFlag);
		if(checkString == VersionFlag)
		{
			//版本号正确
	...
	{
		var fileArr:Array = getFileNameAndExt(url);
		var fileNameAndKey:Array = getMostPossibleVersions(fileArr[0]);
		
		if(String(fileNameAndKey[1]).length>0){
			//验证是否存在版本串
			var checkString:String = String(fileNameAndKey[1]).replace(versionRegExp,VersionFlag);
			if(checkString == VersionFlag)
			{
				if(String(fileArr[1]).length>0)
					return [fileNameAndKey[0],fileArr[1]].join(".");
				else
					return fileNameAndKey[0];
			}
		}
		return url ;
	}
